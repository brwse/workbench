...
with MCPServerAdapter(server_params) as tools:
    ...
    try:
        ...
        developer = Agent(
            ...
            goal="Write and test Python code in an isolated environment using the available tools",
            backstory="You are an experienced Python developer who uses the Workbench environment to safely write and test code. You MUST use the provided tools to complete tasks.",
            tools=tools,
            verbose=True,
            llm="claude-sonnet-4-5-20250929",  # Sonnet is better for tool calling
            allow_delegation=False,  # Prevent delegation, force tool usage
        )

        # Define a task
        task = Task(
            description="""
            Create a Python script that calculates the factorial of a number using the Workbench tools.
